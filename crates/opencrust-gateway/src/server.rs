...
async fn run_scheduler(state: &AppState) -> Result<()> {
    ...
    for task in tasks {
        ...
        let continuity_key = state.continuity_key(Some(task.user_id.as_str())).map(|k| k.as_str().to_string());
        ...
        // 3. Send Response to Channel
        if let Some(channel) = state.channels.get(channel_type.as_str()) {
            let content = MessageContent::Text(response_text.clone());
            ...
            tracing::warn!("Scheduled task for unknown channel type: {}", channel_type);
        }
